...
function TaskCard({ name, description, deadline }: TaskCardProps) {
  ...
  function formatarData(dataISO:Date) {
    ...
    return (
      <div className="grid grid-rows-7 bg-white border border-gray-200 rounded-lg shadow-md pb-3 flex flex-col">
      {/* Título e Data */}
      ...
      {/* Descrição */}
      <div className="row-start-3 row-end-7 px-6 py-3 flex-grow">
        <p className="font-normal text-gray-800">{description}</p>
      </div>

      {/* Botões */}
      <div className="row-start-7 row-end-8 flex justify-end px-6 gap-5">
        <button className="flex bg-green-600 px-4 rounded-lg items-center gap-1 text-white cursor-pointer" onClick={() => setEditModal(true)}>
          <RiPencilFill size={20} />
          <span className="font-semibold">Editar</span>
          <EditTask isOpen={editModal}/>
        </button>
        <button className="flex bg-red-600 px-4 rounded-lg items-center gap-1 text-white cursor-pointer" onClick={() => setDeleteModal(true)}>
          <FaTrashAlt />
